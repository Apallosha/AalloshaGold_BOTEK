    ...
    if msg.from_user.id in admin_states:
        ...
        if state == "add_task":
            try:
                ...
                data["tasks"][tid] = {"channel": channel, "reward": int(reward)}
                save(data)
    ...
    if msg.text == "💰 Баланс":
        ...
        if not data["tasks"]:
            ...
            for tid, t in data["tasks"].items():
                ...
                ikb.add(types.InlineKeyboardButton("✅ Проверить задание", callback_data=f"check_{tid}"))

                bot.send_message(msg.chat.id,
                    f"<b>Задание {tid}!</b>\nПодпишись на {t['channel']} и получи {t['reward']}G",
                    reply_markup=ikb)
